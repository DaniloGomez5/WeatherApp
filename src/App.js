...
function App() {
  ...
  const search = async (city) => {
    const weatherResult = await axios(
      `http://api.openweathermap.org/data/2.5/weather?q=${city}&appid=492fc6a1c14fdf6bdd3130c7e1d7bfa9`
    );
    setWeatherData(weatherResult.data);

    const forecastResult = await axios(
      `http://api.openweathermap.org/data/2.5/forecast?q=${city}&appid=492fc6a1c14fdf6bdd3130c7e1d7bfa9`
    );
